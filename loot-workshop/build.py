...
def compile_pack(root=ROOT, preview=False):
    ...
    routes_path = root / 'global-modifier-routes.json'
    if routes_path.exists():
        routes = read(routes_path)
        require(routes.get('replace') is True, 'Global modifier replacement must be explicit')
        for route in routes['entries']:
            identifier(route)
            require(not route.startswith('perfectplushieapi:'), 'Broken plushie modifier restored')
        files['data/forge/loot_modifiers/global_loot_modifiers.json'] = encoded(routes)
...
def main():
    ...
    for value in plushies['targets'] + plushies['items']: identifier(value)
    require(len(plushies['items']) == len(set(plushies['items'])), 'Duplicate plushie item')
    script = ('// Generated by loot-workshop/build.py. Edit player-plushies.json instead.\n'
        'ServerEvents.chestLootTables(event => {\n'
        '  const items = ' + json.dumps(plushies['items']) + ';\n'
        '  const targets = ' + json.dumps(plushies['targets']) + ';\n'
        '  targets.forEach(id => event.modify(id, loot => {\n'
        '    loot.addPool(pool => {\n'
        '      pool.rolls = 1;\n'
        '      items.forEach(id => pool.addItem(id));\n'
